...
python train.py
    ...
    --learning-rate-aspp 0.00002
    --learning-rate-decoder 0.00002
    --learning-rate-refiner 0
    --checkpoint checkpoint/stage_new/epoch-8.pth
    --checkpoint-dir checkpoint/stage_new
    --log-dir log/stage_new
    --epoch-start 9
    --epoch-end 11
...
nn      --log-dir log/stage3
    --epoch-start 22
    --epoch-end 23

# Stage 4
python train.py --model-variant resnet50 --dataset imagematte --train-hr  --resolution-lr 512 --resolution-hr 2048 --seq-length-lr 40 --seq-length-hr 6 --learning-rate-backbone 0.00001  --learning-rate-aspp 0.00001 --learning-rate-decoder 0.00005 --learning-rate-refiner 0.0002 --checkpoint checkpoint/stage_new/epoch-8.pth --checkpoint-dir checkpoint/stage_new --log-dir log/stage_new --epoch-start 9 --epoch-end 11
"""
import argparse
import torch
import random
...
class Trainer:
        ...
        for epoch in range(self.args.epoch_start, self.args.epoch_end):
            ...
            for true_fgr, true_pha, true_bgr in tqdm(self.dataloader_lr_train, disable=self.args.disable_progress_bar,
                ...
                if self.step % 2 == 0:
                    ...
                    # torch.Size([1, 5, 1, 512, 512])       torch.Size([1, 5, 3, 512, 512])
                else:
    ...
    def train_mat(self, true_fgr, true_pha, true_bgr, downsample_ratio, tag):
        ...
        with autocast(enabled=not self.args.disable_mixed_precision):
            ...
            pred_fgr, pred_pha = self.model_ddp(true_src, gru, downsample_ratio=downsample_ratio)[:2]
    ...
    def train_seg(self, true_img, true_seg, log_label):
        ...
        with autocast(enabled=not self.args.disable_mixed_precision):
            pred_seg = self.model_ddp(true_img,gru, segmentation_pass=True)[0]
